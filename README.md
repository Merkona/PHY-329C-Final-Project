...
Since the time derivative of $n_{A}$ is proportional to current, then if $n_{A}\approx n_{B}$, we get the first Josephson equation, and by combining  ```math 
$$\dot{\varphi}=\dot{\phi}_{B}-\dot{\phi}_{A}$$``` we get the second Josephson equation.
